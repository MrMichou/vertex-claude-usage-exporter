...
def push_metrics_to_gateway(
    ...
    for model, totals in model_totals.items():
        total_requests_gauge.labels(
            model=model,
            date=date_str,
            project=project_id
        ).set(totals['requests'])

        total_cost_gauge.labels(
            model=model,
            date=date_str,
            project=project_id
        ).set(totals['cost'])
...
def main():
    ...
    if args.dry_run:
        ...
        total_cost = 0
        for (email, model), count in sorted(usage.items(), key=lambda x: x[1], reverse=True):
            cost_info = estimate_cost(count, model, args.avg_input_tokens, args.avg_output_tokens, use_calibrated)
            total_cost += cost_info['cost_usd']
            logger.info("  %s / %s: %d requests, $%.4f", email, model, count, cost_info['cost_usd'])
