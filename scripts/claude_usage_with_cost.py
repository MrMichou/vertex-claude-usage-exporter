...
def estimate_cost(
    request_count: int, model_name: str, use_calibrated: bool = True
) -> dict:
...
def main():
    ...
    logger.info(
        "Estimation basee sur: %d input + %d output tokens/requete",
        AVG_TOKENS_PER_REQUEST["input"],
        AVG_TOKENS_PER_REQUEST["output"],
    )
    ...
    logger.info(
        "Pour des couts precis, activez le Request-Response Logging vers BigQuery."
    )
