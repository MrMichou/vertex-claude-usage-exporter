...
def aggregate_usage(entries: list) -> dict:
    ...
    if skipped:
        logger.info("Skipped %d non-Claude entries out of %d total", skipped, len(entries))

    return dict(usage)


def generate_report(usage: dict, target_date: datetime, output_format: str, output_file: str):
...
def main():
    ...
    if not entries:
        ...
        logger.warning("  - Les Data Access Audit Logs sont actives pour aiplatform.googleapis.com")
